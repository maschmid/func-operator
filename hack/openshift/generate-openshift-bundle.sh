...
Options:
  ...
  --source-branch BRANCH                   Git branch to read config/ from (default: main)
...
# Create temporary working directory with config/ from the source branch
WORK_DIR=$(mktemp -d)
trap 'rm -rf "${WORK_DIR}"' EXIT

echo "Extracting source files from branch '${SOURCE_BRANCH}'..."
git -C "${REPO_ROOT}" archive "${SOURCE_BRANCH}" | tar -x -C "${WORK_DIR}"
...
# Copy generated bundle to REPO_ROOT (replacing any existing bundle/)
rm -rf "${REPO_ROOT}/bundle"
cp -r "${WORK_DIR}/bundle" "${REPO_ROOT}/bundle"

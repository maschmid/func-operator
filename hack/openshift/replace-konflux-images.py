#!/usr/bin/env python3
"""Replace Konflux image repositories in the bundle CSV with registry sources."""
...
def read_mapping(path: Path) -> dict[str, str]:
    """Read mirror-to-source pairs from the ImageDigestMirrorSet YAML."""
    text = path.read_text(encoding="utf-8")
    mapping = {
        match.group("mirror"): match.group("source")
...
def replace_images(csv: str, mapping: dict[str, str]) -> tuple[str, int]:
    ...
    def replace(match: re.Match[str]) -> str:
        ...
        source = mapping.get(repository)
        if source is None:
            return match.group(0)
        replacements += 1
        return f"{source}@{match.group('digest')}"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Replace mapped quay.io image repositories in an operator bundle CSV."
    )
    parser.add_argument(
        "--mapping",
        type=Path,
